...
st.markdown("<h3></h3>", unsafe_allow_html=True)
st.markdown("<h3></h3>", unsafe_allow_html=True)
st.markdown("<h3>Project 2: Linear Regression Model for Predicting Embodied Carbon in Buildings</h3>", unsafe_allow_html=True)
st.markdown("<h3></h3>", unsafe_allow_html=True)
st.markdown("<h3>Situation</h3>", unsafe_allow_html=True)
st.write("The construction industry is facing a pressing challenge of reducing its carbon footprint and becoming more "
         "environmentally responsible. The UK Government has mandated targets for reducing carbon emissions, and clients "
         "are increasingly conscious of their environmental impact and reputation. As a result, sustainability has become "
         "a critical consideration in the planning and design of buildings. Aecom, as a leading engineering and consulting "
         "firm, recognizes the importance of helping clients reduce their carbon emissions and meet sustainability targets. "
         "Therefore, developing a tool to predict embodied carbon in buildings is crucial in achieving these "
         "goals and staying ahead of the competition.")

st.write("This predictive model will be an invaluable tool for Aecom and its clients, as the best time to make an impact "
         "on embodied carbon is at the very start of a project. By having the ability to predict the carbon output based "
         "on a set of variables, architects and engineers can make informed decisions about materials and design "
         "choices that will have the most significant impact on reducing a building's carbon footprint.")

st.markdown("<h3></h3>", unsafe_allow_html=True)

st.markdown("<h3>Task</h3>", unsafe_allow_html=True)

st.write(" The task at hand is to create a linear regression model that can predict the embodied carbon in buildings "
         "based on data extracted from BIM models. The goal of this project is to provide a tool that can help Aecom "
         "and its clients make more informed decisions about building materials and design choices, ultimately reducing "
         "the environmental impact of construction projects")

st.write("The construction industry is responsible for a significant portion of global carbon emissions, and reducing "
         "embodied carbon in buildings has become a pressing issue. In response, the government has implemented mandates "
         "and clients are increasingly conscious of their environmental impact and reputation. Aecom, as a leading global "
         "engineering and construction firm, has a responsibility to address this issue and provide solutions for its clients.")

st.write("The proposed solution to this problem is the creation of a linear regression model that can predict embodied carbon "
         "in buildings. By using data extracted from BIM models, the model can identify which building materials and "
         "design choices have the greatest impact on embodied carbon. This information can then be used to inform "
         "decision-making at the earliest stages of construction projects, ultimately reducing the environmental "
         "impact of the final building.")

st.write("The commercial impact of this project is significant. Aecom's clients are increasingly "
         "demanding sustainable solutions, and the ability to predict embodied carbon in buildings is a valuable tool "
         "that can help the company stay competitive in the industry. By providing clients with data-driven insights, "
         "Aecom can position itself as a leader in sustainable construction and differentiate itself from competitors. "
         "Additionally, by reducing the environmental impact of construction projects, Aecom can help clients meet "
         "government mandates and enhance their reputation as responsible corporate citizens.")

st.markdown("<h3>Actions:</h3>", unsafe_allow_html=True)
st.markdown("<h4>Planning:</h4>", unsafe_allow_html=True)

st.write("I held a meeting with internal stakeholders, including the creators of Eco.Zero, to gather as much information "
         "as possible about calculating embodied carbon in buildings. Aecom's Eco.Zero tool utilizes extensive data "
         "collated from external sources to produce a theoretical model using calculations and assumptions. I aimed to "
         "extract this data and gain insights from stakeholders who have a profound understanding of carbon in the "
         "business context and its commercial impact. At the meeting, we discussed their desired tool, and they expressed "
         "a keen interest in continuing with the interactive web page demonstrated in project one as it is the most "
         "effective way to encourage widespread usage.")

st.write("My plan is to use the databases produced by the data extraction tool and filter out the relevant parameters "
         "and component volumes required to calculate the embodied carbon for the building. By applying the relevant "
         "material carbon rates to the building components, we can generate a carbon rate for each building. "
         "Since the volumes of all building components are stored in the databases, we can determine which "
